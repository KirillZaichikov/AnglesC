...
int main(){
    ...
    int paramsDim = 6;
    // double params[] = {9, 25, 2.666666666666, 0, 0.01};
    ...
    double step = 0.01;
    double timeSkip = 100;
    double timeSkipClP = 70;
    double calcTime = 10000;
    ...
    for (int i = 0; i < DISCR; i++)
        for(int j = 0; j < DISCR; j++){
            ...
            double paramsAll[] = {param1[i], param2[j], params[2], params[3], params[4], params[5]};
            if (dimension / 2 <= essDim) {
            ...
            fout<<param1[i]<<" "<<param2[j]<<" "<<lExp1[0]/samplesCounter<<" "<< minPhi <<", ";
        }
